...
async function probeClaudeHealth(): Promise<boolean> {
  ...
  // which works with both AI Agent and OpenAI-compatible backends.
  try {
              ...
              void fetch('/api/start-claude', {
                ...
                .then(async (res) => {
                  ...
                  if (res.ok && data.ok) {
                    setMessage(
                      data.message ||
                        'Auto-restarting AI Agent gateway…',
                    )
  ...
  async function handleStartAgent(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok || !payload.ok) {
        throw new Error(payload.error || 'Failed to start AI Agent')
      }

      setMessage(
        payload.message === 'already running'
          ? 'AI Agent is already running'
          : 'Starting AI Agent…',
      )
    } catch (error) {
      setMessage(
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold">
              {isDisconnected ? 'AI Agent not connected' : 'Connected'}
            </p>
            {message ? (
