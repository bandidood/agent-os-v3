...
export function ProviderWizard({
  ...
  async function handleSaveApiKey() {
    ...
    try {
      ...
      setVerifyState('checking')
      setVerificationMessage(
        `${providerName} API key saved. AI Agent is restarting…`,
      )
      setStep('verify')
      ...
      if (!pollingRef.current) {
        ...
        if (found) {
          ...
          setVerificationMessage(
            `AI Agent restarted, but ${providerName} models haven't appeared yet. ` +
                ...
                {selectedAuthType === 'oauth' ? (
                        ...
                        onClick={function onLaunchOAuth() {
                          ...
                          setVerificationMessage(
                            'Run "hermes setup" in the terminal and select Google OAuth when prompted. ' +
                              'A browser window will open for sign-in. Once complete, AI Agent will restart automatically.',
                          )
                          setVerifyState('warning')
                          setStep('verify')
                        }}
                  ...
                  <>
                    <p className="mt-1 text-sm text-primary-600 text-pretty">
                      ...
                      AI Agent can use the same auth token. Run the configure
                    ...
                    <div className="mt-4 flex flex-col gap-3">
                      ...
                      {saveState === 'saved' ? (
                        <p className="text-xs text-green-600">
                          ...
                          />
                          Key saved! AI Agent is restarting to apply changes.
                        </p>
                      ) : null}
                    </div>
            ...
            {step === 'verify' ? (
              <section className="mt-5">
                ...
                >
                  ...
                  <p className="mt-1 text-sm text-primary-600 text-pretty">
                    {verificationMessage || 'Waiting for AI Agent to respond…'}
                  </p>
                </div>
